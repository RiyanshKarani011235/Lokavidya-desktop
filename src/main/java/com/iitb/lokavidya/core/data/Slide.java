...
public class Slide implements Serializable {
	...
	public Slide(String sourceImageURL,String sourceAudioURL,String projectURL) {
		...
		try {
			FileUtils.copyFile(new File(sourceImageURL), new File(projectURL,imageName+"."+FilenameUtils.getExtension(sourceImageURL)));
			this.imageURL=new File(projectURL,imageName+"."+FilenameUtils.getExtension(sourceImageURL)).getAbsolutePath();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public Slide(String sourceImageURL,String projectURL) {
		...
		GeneralUtils.convertImageToPresentation(sourceImageURL, presentationURL);
		this.setImageURL(new File(projectURL,imageName+"."+FilenameUtils.getExtension(sourceImageURL)).getAbsolutePath());
		populateProperties(sourceImageURL);	
	}
	...
	public Slide(String projectURL) {
		try {
			...
			if(new File(projectURL,presentationName).exists())
			{
				...
				this.imageURL = new File(projectURL,FilenameUtils.getBaseName(presentationName)+".png").getAbsolutePath();
